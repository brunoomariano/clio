...
func TestNoteItemFields(t *testing.T) {
	...
	note := &model.Note{ID: "1", Title: "Title", Tags: []string{"a", "b"}, ExpiresAt: &ex, Body: "Body line"}
	item := noteItem{note: note}
	if item.Title() != "Title" {
		t.Fatalf("unexpected title: %s", item.Title())
	}
	desc := item.Description()
	if !strings.Contains(desc, "Body") || !strings.Contains(desc, "a, b") || !strings.Contains(desc, "expires") {
